...
//      newTau->makePrivateStore(*tauItr);
  ...
  top::check(m_asgHelper->evtStore()->record(tthevt->selected_OR_muons, "SelectedORMuons"),"Could not record Selected Muons after overlap removal");

  std::sort (tthevt->selected_OR_jets->begin(), tthevt->selected_OR_jets->end(), ttHMLAsgHelper::pt_sort());
  top::check(m_asgHelper->evtStore()->record(tthevt->selected_OR_jets, "SelectedORJets"),"Could not record Selected Jets after overlap removal");

  std::sort (tthevt->selected_OR_taus->begin(), tthevt->selected_OR_taus->end(), ttHMLAsgHelper::pt_sort());
  top::check(m_asgHelper->evtStore()->record(tthevt->selected_OR_taus, "SelectedORTaus"),"Could not record Selected Taus after overlap removal");
