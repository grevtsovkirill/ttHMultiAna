...
namespace top{
  ...
  void CustomEventSaver::initialize(std::shared_ptr<top::TopConfig> config, TFile* file, const std::vector<std::string>& extraBranches)
  {
    ...
    for (auto systematicTree : treeManagers()) {
      ...
      std::cout<<systematicTree->name()<<std::endl;
      Variables(systematicTree);
    }
  }
  void CustomEventSaver::Variables(std::shared_ptr<top::TreeManager> systematicTree){
    std::shared_ptr<ttHML::Event> tthevt;
    tthevt->BootstrapTree(systematicTree,false);
  }
